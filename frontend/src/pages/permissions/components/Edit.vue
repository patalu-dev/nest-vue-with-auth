...
watch(isOpen, (newVal) => {
    if (newVal && props.permission) {
        ...
        conditions.value = props.permission.conditions ? JSON.stringify(props.permission.conditions, null, 2) : ''
        inverted.value = props.permission.inverted || false
        errors.value = { action: '', subject: '', conditions: '', name: '' }
        
        // Fetch action and subject options only when editing
        fetchOptions()
    }
})
        ...
        <DialogContent class="sm:max-w-[500px]">
            ...
            <div class="mt-3 grid gap-4 max-h-[70vh] overflow-y-auto px-1 text-left">
                ...
                <div class="grid gap-2 p-3 border rounded-md bg-gray-50/30">
                    <Label class="text-gray-800 font-medium mb-1">Hành động (Actions) <span class="text-red-500">*</span></Label>
                    <div class="grid grid-cols-2 gap-3">
                        <div v-for="opt in actionOptions" :key="opt" class="flex items-center space-x-2">
                            <input 
                                ...
                                :id="'edit-action-' + opt"
                                :value="opt"
                                :checked="action.split(',').map(a => a.trim()).includes(opt)"
                                @change="(e) => {
                            ...
                            />
                            <label :for="'edit-action-' + opt" class="text-sm cursor-pointer select-none">{{ opt }}</label>
                        </div>
                    </div>
                    <div class="mt-2 pt-2 border-t border-dashed">
                        <Label class="text-[11px] text-gray-500 uppercase">Hoặc nhập thủ công (ngăn cách bằng dấu phẩy):</Label>
                        <Input v-model="action" placeholder="VD: custom_action, another_one" class="h-8 text-xs mt-1" :disabled="loading" />
                    </div>
                ...
                <div class="flex items-center justify-between space-x-2 rounded-lg border p-3 bg-amber-50/30 border-amber-200">
                    <div class="space-y-0.5 flex-1">
                        <Label for="inverted-mode-native-edit" class="text-base cursor-pointer font-bold text-amber-900">Chế độ cấm (Inverted)</Label>
                        <p class="text-[13px] text-amber-700">
                            Bật để tạo quyền "KHÔNG ĐƯỢC PHÉP". Trạng thái: <span class="font-bold underline">{{ inverted ? 'ĐANG BẬT' : 'ĐANG TẮT' }}</span>
                        </p>
                    </div>
                    <input 
                        type="checkbox" 
                        id="inverted-mode-native-edit" 
                        v-model="inverted" 
                        :disabled="loading" 
